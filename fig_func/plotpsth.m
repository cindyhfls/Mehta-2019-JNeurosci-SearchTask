...
% close all
% clear
% load('../input_data/data_wrapped_Cindy.mat','data');
% numneuron=size(data,2);
% %% params:
% binWidth = 10; % in ms, of psth_alignedBarView
% epochOI = 500:1500;
...
%% pull out psth_alignedBarView's

smoothBy = 10;
figure('Position',[440   461   785   337]); hold on; set(gca,'FontSize',10);hold on
...
for c = 1:length(data)
    psth_alignedBarView = data(c).psth_alignedBarView(:,epochOI)*(1000/binWidth);
    
    % normalize within cell (zscore to mean and std of FR across trials):
    psth_alignedBarView = (psth_alignedBarView - nanmean(nanmean(psth_alignedBarView,2)))./nanste(nanmean(psth_alignedBarView,2));
    %     theta = get_egocentric_tuning(data,c,epochOI);
    [~,~,~,theta]=get_xyspike_priyanka(data,c,epochOI);
    [~,~,bin] = histcounts(theta,edges);
    for q = 1:divideint-1
        idx = bin == q;
        m(c,:,q) = nanmean(psth_alignedBarView(idx,:));
        count(c,q) = sum(idx);
        se(c,:,q) = nanste(psth_alignedBarView(idx,:)); % just one neuron
    end
end
%% plot psth_alignedBarView
